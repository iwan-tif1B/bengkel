...
@section('content')
                ...
                swalWithBootstrapButtons.fire({
                    ...
                    if (result.isConfirmed) {
                        window.location.href =
                            "{{ url('status') }}"; // Ganti 'redirect-page' dengan URL tujuan Anda
                    }
                });
            });
        </script>
                ...
                <div class="col-md-12 grid-margin stretch-card" style="margin-left:100px;">
                    <div class="card">
                        <div class="card-body">
                            ...
                            <form class="forms-sample" action="{{ route('buku_tamu.store') }}" method="POST"
                                enctype="multipart/form-data" onsubmit="return confirmFormSubmit(event)">
                                @csrf
                                ...
                                <div class="form-group">
                                    ...
                                    <input type="text" id="no_hp" name="no_hp" class="form-control"
                                        placeholder="No HP" maxlength="12" pattern="\d{1,12}"
                                        title="Only numbers are allowed, up to 12 digits" oninput="validateInput(this)">
                                </div>
                                <script>
                                ...
                                <div class="form-group">
                                    ...
                                    <input type="date" class="form-control tanggal_buku_tamu"
                                        onchange="change_tanggal()" name="tanggal" min="{{ $availableDates[0] }}"
                                        value="{{ $availableDates[0] }}">
                                    <div class="text-warning jumlah_buku_tamu_hari_ini">Total Jumlah Booking :
                                        {{ $hitung_hari_ini }}</div>
                                </div>

                                <div class="form-group">
                                    <label for="exampleInputUsername1">Upload Foto Kendaraan<span
    ...
    <span class="url_check_buku_tamu" data-url="{{ route('buku_tamu.create') }}"></span>
    <script>
        // count buku tamu

        function change_tanggal() {
            var tanggalInput = $(".tanggal_buku_tamu");
            var tanggal = tanggalInput.val();

            $.ajax({
                url: $(".url_check_buku_tamu").data("url"),
                method: 'GET',
                data: {
                    tanggal: tanggal
                },
                success: function(response) {
                    if (response.jumlah_tamu > 4) {
                        // Jika jumlah booking lebih dari 4, ganti tanggal ke hari berikutnya
                        var newTanggal = getNextDate(tanggal);
                        tanggalInput.val(newTanggal);
                        updateMinDate(newTanggal);
                        change_tanggal(); // Cek ulang dengan tanggal baru
                    } else {
                        $(".jumlah_buku_tamu_hari_ini").text("Total Jumlah Booking : " + response.jumlah_tamu);
                    }
                },
                error: function(xhr) {
        ...
        function getNextDate(currentDate) {
            ...
            date.setDate(date.getDate() + 1); // Set ke hari berikutnya
            return date.toISOString().split('T')[0]; // Format tanggal menjadi YYYY-MM-DD
        }

        function updateMinDate(date) {
            $(".tanggal_buku_tamu").attr("min", date);
        }



        // $(document).(".tanggal_buku_tamu").on("change", function() {
        //     var tanggal = $(this).val();

        // });


        // function check_jumlah_hari_ini() {
        //     $.ajax({
        //         url: $(".url_check_buku_tamu").data("url"),
        //         success: function(response) {
        //             if (response.jumlah_tamu > 0) {
        //                 $(".tanggal_buku_tamu").hide()
        //             }
        //         }
        //     })
        // }
        // setInterval(check_jumlah_hari_ini, 5000);
        // end count buku tamu
